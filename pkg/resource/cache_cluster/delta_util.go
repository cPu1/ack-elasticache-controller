...
import (
	...
	"github.com/aws-controllers-k8s/elasticache-controller/apis/v1alpha1"
...
// updateLDCsDelta retrieves the last requested LogDeliveryConfigurations saved in annotations and compares them
// to the current desired configurations. If a diff is found, it adds it to delta.
func updateLDCsDelta(desired *resource, delta *ackcompare.Delta) {
	var lastRequestedLDCs []*v1alpha1.LogDeliveryConfigurationRequest
	unmarshalAnnotation(desired, AnnotationLastRequestedLDCs, &lastRequestedLDCs)
	if !reflect.DeepEqual(desired.ko.Spec.LogDeliveryConfigurations, lastRequestedLDCs) {
